...
const server = http.createServer((req, res) => {
    ...
    if (req.method === 'GET' && req.url === '/paises') {
        ...
        for(let i=0; i < paises.length; i++){
            informacion += JSON.stringify(paises[i].nombre);
            informacion += "Indice del país:" + i;
        }
        res.end(JSON.stringify({info: informacion, paises}));
        ...
        if (indice >= 0 && indice < paises.length) {
            ...
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ mensaje: 'País eliminado', paises }));
        } else {
            res.writeHead(400, { 'Content-Type': 'application/json' });
